...
**Fmask 4.2 softwares** (including standalones with GUI and without GUI) on Windows, Mac, and Linux (~1G) are also ready to use now! It can be downloaded at this [Google Drive](https://drive.google.com/drive/folders/1bVwvlGDFOsWnVj5b3MqI5yqRDoi8g935?usp=sharing). Note the standalones require Internet Connection to download [Matlab Runtime](https://www.mathworks.com/help/compiler/install-the-matlab-runtime.html). Also, you can contact us to request the version with Matlab Runtime (~3G), which can be installed without Internet.
