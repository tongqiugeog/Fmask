...
%         if FilesInfo.Granule(1) == 'L'
%             tempfn = dir(fullfile(Dmain, [FilesInfo.Granule(5:10) '*_B' BandCorr{iB} '.jp2']));
% %             if isequal(iB,4)
% %                 [dum,~,~,info_jp2]=geoimread([Dmain tempfn(1).name]);
% %             else
%                 dum=imread(fullfile(Dmain, tempfn(1).name));
% %             end
%         else
% %             if isequal(iB,4)
% % %                 [dum,~,~,info_jp2]=geoimread('/Volumes/Transcend/Fmask3.3S2/Test data/S2A_OPER_PRD_MSIL1C_T31TCJ_2016292A.SAFE/GRANULE/S2A_OPER_MSI_L1C_TL_SGS__20161018T175658_A006912_T31TCJ_N02.04/IMG_DATA/S2A_OPER_MSI_L1C_TL_SGS__20161018T175658_A006912_T31TCJ_B05.jp2');
% %                 [dum,~,~,info_jp2]=geoimread([Dmain FilesInfo.Granule(1:end-7) 'B' BandCorr{iB} '.jp2']);  
% %             else
%                 %         dum=imread([Dmain FilesInfo.Granule(1:end-6) 'B' BandCorr{iB} '.jp2']);
%                 dum=imread(fullfile(Dmain, [FilesInfo.Granule(1:end-7) 'B' BandCorr{iB} '.jp2']));  
% %             end
%         end
